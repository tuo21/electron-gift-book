...
import { dbRecordListToRecordList, recordToDbRecord } from './utils/recordMapper';
...
const toastRef = shallowRef<InstanceType<typeof Toast>>();
...
const loadRecords = async () => {
  try {
    ...
    if (response.success && response.data) {
      records.value = dbRecordListToRecordList(response.data as any).reverse();
...
// 功能处理函数
const handleSave = () => { 
  toastRef.value?.success('数据已自动保存'); 
};
...
const handleExportExcel = async () => {
  if (records.value.length === 0) {
    toastRef.value?.warning('没有可导出的记录');
    return;
  }
...
const performSearch = async () => {
  ...
  try {
    ...
    if (response.success && response.data) {
      searchResults.value = dbRecordListToRecordList(response.data as any);
    } else {
