...
function setupIpcHandlers() {
  ...
  ipcMain.handle("electron:parseImportFile", async (_, filePath) => {
    try {
      ...
      try {
        ...
        return { success: false, error: "读取文件失败: " + readError.message };
      }
      let workbook;
      try {
        workbook = readSync(binaryString, { type: "binary" });
        console.log("Excel 解析成功，工作表:", workbook.SheetNames);
      } catch (xlsxError) {
        console.error("解析 Excel 失败:", xlsxError);
        return { success: false, error: "解析 Excel 失败: " + xlsxError.message };
      }
      ...
      const data = jsonData.slice(1).filter((row) => row.some((cell) => cell !== void 0 && cell !== ""));
  ...
  ipcMain.handle("db:batchInsertRecords", async (_, records) => {
    const db2 = getDatabase();
    try {
      const insertedIds = [];
      const transaction = db2.transaction(() => {
